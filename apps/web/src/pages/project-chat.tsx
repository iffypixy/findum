...
import {ChatMessage, Nullable, ProjectChat} from "@shared/lib/types";
...
export const ProjectChatPage: React.FC = () => {
  ...
  const [currentTab, setCurrentTab] = useState<Tab>("rooms");
  ...
  const [chat, setChat] = useState<Nullable<ProjectChat>>(null);

  const [messages, setMessages] = useState<Nullable<ChatMessage[]>>(null);

  useEffect(() => {
    api.chats.getProjectChat({projectId}).then(({data}) => {
      setChat(data.chat);
          ...
          >
            <div className="flex items-center justify-between bg-[#CBD2E4] p-6">
              <div
                role="presentation"
                onClick={() => {
                  navigate(`/projects/${chat.project.id}`);
                }}
